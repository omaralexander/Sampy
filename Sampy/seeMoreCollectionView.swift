...
extension seeMore: UISearchResultsUpdating {
...
class seeMore: UIViewController,UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout,UISearchBarDelegate,UITextFieldDelegate{
    ...
    override func viewDidLoad() {
        ...
        if UIDevice().userInterfaceIdiom == .phone{
            ...
            case 2436:
                ...
                viewForCell.frame.size.height = 190
                   viewForSearchBar.frame.origin.y = 11
                //   imageButton.frame.origin.y = 53
                //  dismissButton.frame.origin.y = 41
     ...
     print("this is the height \(heightForViewForCell)")
        
        viewForCell.frame.size.height = heightForViewForCell
        viewForCell.frame.size.width = widthForViewForCell
        
       print("this is the frame for viewFroCell]\(viewForCell.frame)")
    ...
    func fetchUser(){
        ...
        ref.queryOrdered(byChild: "post").observe(.childAdded, with: {(snapshot) in
            if let dictionary = snapshot.value as? [String: AnyObject]{
                ...
                let dictionaryViewCount = dictionary["viewCount"] as! NSNumber
                let type = dictionary["type"] as! String
              
                
                let structure = MapPoints(Latitude: lat, Longitude: long, Image: image, Text: text, User: user, PostCity: address, Time: time, userUID: uid, ViewCount: dictionaryViewCount, Type: type)
